...
export const generateSong = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const { userId, credits, endpoint, body } = (await step.run(
      ...
      async () => {
        ...
        // Custom mode: Prompt + described lyrics
          ...
          endpoint = env.GENERATE_WITH_DESC_AND_LYRICS;
          body = {
            desc_lyrics: song.describedLyrics,
            prompt: song.prompt,
            ...commomParams,
          };
        }
    ...
    )) as { userId: string; credits: number; endpoint: string; body: RequestBody };
      ...
      await step.run("update-song-result", async () => {
        ...
        await db.song.update({
          ...
          data: {
            ...
            thumbnailS3Key: responseData?.cv_image_s3_key,
